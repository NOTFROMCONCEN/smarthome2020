package com.example.shengsai2019e0921.view;
...
import com.example.shengsai2019e0921.fragment.BaseFragment;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (true) {
					...
					if (!BaseFragment.string_name.isEmpty()) {
						data.add(BaseFragment.getdata);
						Xlable.add(String.valueOf(Integer
								.valueOf(BaseFragment.number) + num));
						num++;
	...
	protected void onDraw(Canvas canvas) {
		...
		if (data.size() > 1) {
			for (int i = 0; i < data.size(); i++) {
				if (data.get(i) >= 100) {
					canvas.drawRect(Xpoint + i * Xheight + 5, Ypoint - 5
							* Yheight, Xpoint + (i + 1) * Xheight - 5, Ypoint,
							paint);
				}
				if (data.get(i) < 100) {
					canvas.drawRect(Xpoint + i * Xheight + 5,
							Ypoint - data.get(i) * Yheight / 20, Xpoint
									+ (i + 1) * Xheight - 5, Ypoint, paint);
				}
				canvas.drawText(Xlable.get(i), Xpoint + i * Xheight + 10,
						Ypoint + 20, paint);
